...
# parameters
parser.add_argument(
    '--samples_file', default="../data/samples_S2S5P_2018_2020_eea.csv", type=str)
parser.add_argument(
    '--datadir', default="/netscratch/lscheibenreif/eea", type=str)
...
parser.add_argument(
    '--checkpoint', default="../checkpoints/pretrained_resnet50_LUC.model", type=str)
...
parser.add_argument('--dropout_p_second_to_last_layer',
...
parser.add_argument('--log_transform', default="True", type=str)

args = parser.parse_args()
config = dotdict(
    {k: strtobool(v) if k in bool_args else v for k, v in vars(args).items()})

sources = config.samples_file.split("_")[1]
frequency = "2018_2020" if "2018" in config.samples_file else config.samples_file.split("_")[
    2].replace(".csv", "")
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
checkpoint_name = "pretrained" if config.checkpoint is not None else "from_scratch"

experiment = "_".join([datetime.today().strftime(
    '%Y-%m-%d-%H:%M'), sources, checkpoint_name, frequency])
if config.verbose:
    print("Initializing mlflow experiment:", experiment)
...
for run in tqdm(range(1, config.runs+1), unit="run"):
    ...
    with mlflow.start_run(experiment_id=experiment_id):
        ...
        mlflow.log_param("dropout_p_second_to_last_layer",
                         config.dropout_p_second_to_last_layer)
        ...
        mlflow.log_param("log_transform", config.log_transform)
        ...
        samples_train, samples_val, samples_test, stations_train, stations_val, stations_test = split_samples(
            samples, list(stations.keys()), 0.2, 0.2)
        if config.verbose:
            print("First stations_train:", list(stations_train)[:10])
        dataset_test = NO2PredictionDataset(
            config.datadir, samples_test, frequency, sources, transforms=tf, station_imgs=stations)
        dataset_train = NO2PredictionDataset(
            config.datadir, samples_train, frequency, sources, transforms=tf, station_imgs=stations)
        dataset_val = NO2PredictionDataset(
            config.datadir, samples_val, frequency, sources, transforms=tf, station_imgs=stations)
        dataloader_train = DataLoader(
            dataset_train, batch_size=config.batch_size, num_workers=4, shuffle=True, pin_memory=False)
        dataloader_test = DataLoader(
            dataset_test, batch_size=1, num_workers=1, shuffle=False, pin_memory=False)
        dataloader_val = DataLoader(
            dataset_val, batch_size=1, num_workers=1, shuffle=False, pin_memory=False)
        dataloader_train_for_testing = DataLoader(
            dataset_train, batch_size=1, num_workers=1, shuffle=False, pin_memory=False)

        if config.verbose:
            print("Initializing model")
        model = get_model(sources, device, config.checkpoint,
                          dropout=dropout_config, heteroscedastic=config.heteroscedastic)
        model.to(device)
        optimizer = optim.Adam(model.parameters(
        ), lr=config.learning_rate, weight_decay=config.weight_decay_lambda)
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, 'min', patience=4, factor=0.5, threshold=1e6, min_lr=1e-7)
        ...
        for epoch in range(config.epochs):
            ...
            if epoch == 5 and config.heteroscedastic:
                if config.verbose:
                    print("Changing to heteroscedastic loss...")
            ...
            for idx, sample in enumerate(dataloader_train):
                ...
                loss_batch, metric_results = step(
                    model_input, y, model, loss, optimizer, config.heteroscedastic)
            ...
            val_y, val_y_hat = test(sources, model, dataloader_val,
                                    device, datastats, config.dropout, config.heteroscedastic, config.log_transform)

            valid_val = (val_y_hat < 100) & (val_y_hat > 0)
            eval_val = eval_metrics(val_y, val_y_hat)

            if config.verbose:
                print("Fraction of valid estimates:",
                      sum(valid_val)/len(valid_val))
        ...
        test_y, test_y_hat = test(sources, model, dataloader_test,
                                  device, datastats, config.dropout, config.heteroscedastic, config.log_transform)
        train_y, train_y_hat = test(sources, model, dataloader_train_for_testing,
                                    device, datastats, config.dropout, config.heteroscedastic, config.log_transform)
        ...
        # log everything that was written to the artifacts directory
        mlflow.log_artifacts("artifacts")

    performances_val = pd.DataFrame(performances_val, columns=[
                                    "run", "epoch", "r2", "mae", "mse"])
    performances_test = pd.DataFrame(
        performances_test, columns=["r2", "mae", "mse"])
    performances_train = pd.DataFrame(
        performances_train, columns=["r2", "mae", "mse"])
...
performances_test.to_csv(os.path.join(config.result_dir, "_".join([sources, str(
    checkpoint_name), frequency, "test", str(config.epochs), "epochs"]) + ".csv"), index=False)
performances_train.to_csv(os.path.join(config.result_dir, "_".join([sources, str(
    checkpoint_name), frequency, "train", str(config.epochs), "epochs"]) + ".csv"), index=False)
...
torch.save(model.state_dict(), os.path.join(config.result_dir, "_".join(
    [sources, str(checkpoint_name), frequency, str(config.epochs), "epochs"]) + ".model"))
if config.verbose:
    print("done.")
