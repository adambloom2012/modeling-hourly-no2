...
class NO2PredictionDatasetWithDataloading(Dataset):
    ...
    def __init__(self, datadir, samples, frequency, sources, preload=False, transforms=None):
        ...
        if self.frequency != "2018_2020":
            sample = self.sample_df.iloc[0]
            s5p_sample = xr.open_dataset(os.path.join(
                self.datadir, "sentinel-5p", sample["s5p_path"])).rio.write_crs(4326)
            if self.frequency == "quarterly":
                self.s5p_dates = np.array(["Q-" + str(dt.quarter) + "-" + str(dt.year)
                                          for dt in pd.to_datetime(s5p_sample.time.values)])
            elif self.frequency == "monthly":
                self.s5p_dates = np.array(
                    [str(dt.month) + "-" + str(dt.year) for dt in pd.to_datetime(s5p_sample.time.values)])
            elif self.frequency == "hourly":
                self.s5p_dates = np.array(
                    [str(dt.month) + "-" + str(dt.day) + "-" + str(dt.hour) for dt in pd.to_datetime(s5p_sample.time.values)])
            s5p_sample.close()
    ...
    def load_data_to_memory(self, sample):
        if sample.get("img") is None:
            sample["img"] = np.load(os.path.join(
                self.datadir, "sentinel-2", sample["img_path"]))
