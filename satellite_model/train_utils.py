...
def split_samples(samples, stations, test_size=0.2, val_size=0.2):
    stations_train, stations_test = train_test_split(
        stations, test_size=test_size)
    real_val_size = val_size / (1 - test_size)
    stations_train, stations_val = map(
        set, train_test_split(stations_train, test_size=real_val_size))
    stations_test = set(stations_test)

    samples_train = [
        s for s in samples if s["AirQualityStation"] in stations_train]
    samples_test = [
        s for s in samples if s["AirQualityStation"] in stations_test]
    samples_val = [
        s for s in samples if s["AirQualityStation"] in stations_val]
    ...
    # if station in stations_train:
    # samples_train.append(copy.deepcopy(sample))
    # elif station in stations_test:
    # samples_test.append(copy.deepcopy(sample))
    # elif station in stations_val:
    # samples_val.append(copy.deepcopy(sample))
...
def train(sources, model, loss, optimizer, scheduler, dataloader, epochs, device, dropout, heteroscedastic):
    model.train()
    if dropout:
    ...
    if "S5P" in sources:
        for epoch in range(epochs):
            ...
            for idx, sample in enumerate(dataloader):
                ...
                model_input = {"img": img, "s5p": s5p, "hour": hour, "day": day, "month": month, "PopulationDensity": population_density, "LocationType": location_type}
...
if __name__ == "__main__":
    ...
    tf = transforms.Compose(
        [ChangeBandOrder(), Normalize(datastats), Randomize(), ToTensor()])
    ...
    for run in tqdm(range(num_runs)):
        ...
        samples_train, samples_val, samples_test = split_samples(
            samples, stations, 0.2, 0.2)
        if verbose:
            print("run:", run)
        ...
        dataset_test = NO2PredictionDataset(
            datadir, samples_test, frequency, sources, transforms=tf)
        dataset_train = NO2PredictionDataset(
            datadir, samples_train, frequency, sources, transforms=tf)
        dataset_val = NO2PredictionDataset(
            datadir, samples_val, frequency, sources, transforms=tf)

        dataloader_train = DataLoader(
            dataset_train, batch_size=batch_size, num_workers=4, shuffle=True, pin_memory=True)
        dataloader_test = DataLoader(
            dataset_test, batch_size=1, num_workers=1, shuffle=False, pin_memory=True)
        dataloader_val = DataLoader(
            dataset_val, batch_size=1, num_workers=1, shuffle=False, pin_memory=True)
        dataloader_train_for_testing = DataLoader(
            dataset_train, batch_size=1, num_workers=1, shuffle=False, pin_memory=True)
        ...
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, 'min', factor=0.5, threshold=1e-5, min_lr=1e-6)
    ...
    performances_train.to_csv(
        "data/performances_train_" + sources + "_" + frequency + ".csv", index=False)
    performances_test.to_csv(
        "data/performances_test_" + sources + "_" + frequency + ".csv", index=False)
    performances_val.to_csv("data/performances_val_" +
                            sources + "_" + frequency + ".csv", index=False)
